...
fn extract_zoom_link(txt: String) -> Option<String> {
    lazy_static! {
        static ref RE: Regex = Regex::new(r"(https?://(.*?zoom\.us)/j/([0-9]+)?pwd=([a-zA-Z0-9]+))").unwrap();
    }

    let n = RE.captures(txt.as_str()).iter().next().map(|c|
        format!("zoommtg://{}/join?action=join&confno={}&pwd={}", c.get(2).unwrap().as_str(), c.get(3).unwrap().as_str(), c.get(4).unwrap().as_str())
    );
